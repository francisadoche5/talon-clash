    ...
    try {
      ...
      for (let y = 0; y < dispH; y++) {
        for (let x = 0; x < dispW; x++) {
          ...
          const nbr = (x > 0 && wasRemoved[p - 1]) || (x < dispW - 1 && wasRemoved[p + 1]) ||
                      (y > 0 && wasRemoved[p - dispW]) || (y < dispH - 1 && wasRemoved[p + dispW]);
          if (!nbr) continue;
          const i = p * 4;
          const avg = (data[i] + data[i + 1] + data[i + 2]) / 3;
          const variance = Math.max(Math.abs(data[i] - data[i + 1]), Math.abs(data[i + 1] - data[i + 2]), Math.abs(data[i] - data[i + 2]));
...
const STYLES = `
  ...
  @keyframes headerDrop   { from{transform:translateY(-30px);opacity:0} to{transform:translateY(0);opacity:1} }
  @keyframes ribbonIn     { from{transform:scaleX(0.3);opacity:0} to{transform:scaleX(1);opacity:1} }
  @keyframes cardRise     { from{transform:translateY(24px);opacity:0} to{transform:translateY(0);opacity:1} }
  ...
  .hdr-drop    { animation: headerDrop 0.5s 0s   cubic-bezier(0.22,1,0.36,1) both; }
  .ribbon-in   { animation: ribbonIn   0.4s 0.12s cubic-bezier(0.22,1,0.36,1) both; }
  .card-rise   { animation: cardRise   0.4s 0.22s cubic-bezier(0.22,1,0.36,1) both; }
...
// Reward tile — label + golden square icon + value
function RewardTile({ label, icon, value, delay = 0 }) {
  ...
  return (
    <div className="reward-pop flex flex-col items-center gap-1" style={{ animationDelay: `${delay}s` }}>
      ...
      <div className="w-[64px] h-[64px] rounded-[18px] flex items-center justify-center"
        style={{
...
export default function BattleArena({ player, result, onClose }) {
  ...
  // Reward tiles — "Feathers" (NOT "Dust"), no Food
    ...
    <div className="fixed inset-0 z-50 flex flex-col select-none"
          ...
          <div className="w-full flex items-end justify-center gap-4 mt-auto">
            ...
            <div className="flex flex-col items-center slide-right">
              ...
              <div className="mt-1 px-2 py-0.5 rounded-full text-xs font-bold border border-red-500 text-red-400"
                style={{ background: '#ef444418' }}>RIVAL</div>
            </div>
          </div>
          {showBattleStart && (
      ...
      {phase === 'result' && (
        <div className="flex-1 flex flex-col" style={{ background: resultBg }}>
          ...
          {/*
            Scrollable content — image sits directly on top of ribbon.
            Continue button is ALWAYS visible below (flex-shrink-0).
          */}
          <div className="flex-1 overflow-y-auto">
            <div className="flex flex-col items-center px-4 pt-4 pb-2">

              {/* ── Header image (swords + crown) ──
                  marginBottom: -80  →  pulls the ribbon UP so image bottom
                  overlaps the top of the ribbon, visually "resting on it". */}
              <div className="hdr-drop w-full flex justify-center"
                style={{ marginBottom: -80, position: 'relative', zIndex: 30 }}>
                <img
                  ...
                  style={{
                    ...
                    objectFit: 'contain',
                    filter: won
                      ? 'drop-shadow(0 6px 20px rgba(255,200,0,0.6)) drop-shadow(0 3px 10px rgba(0,0,0,0.8))'
                      : 'drop-shadow(0 6px 20px rgba(139,92,246,0.5)) drop-shadow(0 3px 10px rgba(0,0,0,0.8))',
                  }}
                />
              </div>

              {/* ── Victory / Defeat ribbon (image overlaps from above) ── */}
              <div className="ribbon-in w-full flex items-center justify-center"
                style={{
                  background: ribbonBg,
              ...
              {/* ── Reward card — sits directly below ribbon, no gap ── */}
              <div className="card-rise w-full"
                style={{
                  background: 'linear-gradient(160deg,#fdf6e0 0%,#ede1b4 60%,#e0d09a 100%)',
                  boxShadow: '0 16px 50px rgba(0,0,0,0.55), inset 0 1px 0 rgba(255,255,255,0.7)',
          ...
          {/* ── Continue button — ALWAYS visible, never hidden ── */}
          <div className="flex-shrink-0 px-4 py-4"
            style={{ background: won ? 'rgba(5,14,0,0.97)' : 'rgba(7,0,2,0.97)' }}>
            <button
